...
function checkAll(){
	...
	user_account.add( Validate.Length, { maximum: 30} );
	user_account.add( Validate.Custom, { failureMessage: '登录名已经存在！', against: function(value, args){
		var valid = 'true';
		var params = {
				...
				<tbody>
					<tr>
						...
						<td>
							<select name="user.user_type" id="user_type" class="downMenu">
								<option value="1" <s:if test="%{user.user_type == 1}">selected="selected"</s:if>>内部用户</option>
								<option value="2" <s:if test="%{user.user_type == 2}">selected="selected"</s:if>>外部用户</option>
							</select>
						</td>
						<th><font>*</font>登录账号</th>
						<td>
							<input type="text" name="user.user_account" id="user_account" value="${user.user_account}"/>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<input type="text" class="chooseInput" title="点击选择信息" onclick="chooseOrgan('drugTypeOne','org_id','org_name','menuContentOne',1,0);" readonly="readonly" name="user.org_name" id="org_name" value="${user.org_name}" style="width:153px;" readonly="readonly"/>
					...
					<tr>
						...
						<td>
							...
							<input type="text" class="chooseInput" title="点击选择信息" onclick="chooseOrgan2('drugTypeTwo','dept_id','dept_name','menuContentTwo',2);" readonly="readonly" name="user.dept_name" id="dept_name" value="${user.dept_name }" style="width:153px;" readonly="readonly"/>
							<input type="hidden" name="user.dept_id" id="dept_id" value="${user.dept_id }" />
						</td>
						<th><font>*</font>性&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;别</th>
						<td>
					...
					<tr>
						...
						<td>
							<select name="user.pos_id" id="pos_id" class="downMenu">
								<option value="${user.pos_id}">${user.pos_name}</option>
							</select>
						</td>
						<th><font>*</font>用户手机号</th>
						<td><input type="text" name="user.cellphone_no" id="cellphone_no" value="${user.cellphone_no}"/></td>
					</tr>
					<tr>
						<th><font>*</font>所在区域</th>
						<td>
							<input type="text" name="user.area_name" id="area_name" class="chooseInput" value="${user.area_name }" title="点击选择信息" onclick="chooseArea('area_id','area_name');" readonly="readonly" />
							<input type="hidden" name="user.area_id" id="area_id" value="${user.area_id }" />
						</td>
