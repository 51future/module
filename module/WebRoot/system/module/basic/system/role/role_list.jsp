...
function changeMode(mode){
	...
	if(mode == 'update' && $('#role_id').val()=='') mode = 'insert';
		...
		<table>
			...
			<tbody>
				<s:iterator value="%{roleList}">
					<tr>
						<td class="tdLeft" title="${role_name}">
							<s:if test="%{role_name !=null && role_name.length()>5}">
								<s:property value="role_name.substring(0,5)+'...'"/>
							</s:if><s:else>${role_name}</s:else>
						</td>
						<td class="tdLeft" title="${org_name}">
							<s:if test="%{org_name !=null && org_name.length()>5}">
								<s:property value="org_name.substring(0,5)+'...'"/>
							</s:if><s:else>${org_name}</s:else>
						</td>
						...
						<td class="tdLeft" title="${remark}">
							<s:if test="%{remark !=null && remark.length()>5}">
								<s:property value="remark.substring(0,5)+'...'"/>
							</s:if><s:else>${remark}</s:else>
						</td>
						<td>
							<s:if test="%{issys==1}">
								<s:if test="%{enable == 2}">
							...
							<s:if test="%{enable ==1}">
								<a href="javascript:void(0);" disabled="disabled" class="audit" >角色授权</a> 
								<a href="javascript:void(0);" disabled="disabled" class="audit" >设置用户</a> 
							</s:if><s:else>
