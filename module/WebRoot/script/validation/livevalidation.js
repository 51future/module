...
var Validate = {
	 ...
	 *	联系电话号码验证
	 */
	MobileTelephone : function(value, paramsObj) {
		var paramsObj = paramsObj || {};
		var message = paramsObj.failureMessage || "无效号码!";
		var suppliedValue = value;
		if ((/^((0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$/.test(suppliedValue))) {
			return true;
		} else if ((/^(13[0-9]|15[0|1|2|3|6|7|8|9]|18[4|5|6|7|8|9])\d{8}$/.test(suppliedValue))) {
			return true;
		} else {
			Validate.fail(message);
		}
	 ...
	 *	身份证号码验证
	 */
	CardIdNo : function(value, paramsObj) {
		var paramsObj = paramsObj || {};
		var message = paramsObj.failureMessage || "号码无效";
		var len = value.length;
		var suppliedValue = value;
		if (len == 15) {
			if ((/^(\d{6})()?(\d{2})(\d{2})(\d{2})(\d{3})$/.test(suppliedValue))) {
				return true;
			} else {
				Validate.fail(message);
			}
		} else if (len == 18) {
			if ((/^(\d{6})()?(\d{4})(\d{2})(\d{2})(\d{3})(\d)$/.test(suppliedValue))) {
				return true;
			} else {
				Validate.fail(message);
			}
